...
def evalExp(gtBin, cur_prob, thres):
    ...
    thresInf = np.concatenate(([-np.Inf], thres, [np.Inf])) # 负无穷大，和正无穷大，便于后续进行直方图统计

    fnArray = cur_prob[(gtBin == True)] # 看GT中变化了的区域，对应预测是多少
    fnHist = np.histogram(fnArray, bins=thresInf)[0]
    fnCum = np.cumsum(fnHist)
    FN = fnCum[0: 0 + len(thres)]

    fpArray = cur_prob[(gtBin == False)]
    fpHist = np.histogram(fpArray, bins=thresInf)[0] # 左闭右开区间，按照阈值划分有多少样本？
    # 倒置求累计和再倒置回去
    fpCum = np.flipud(np.cumsum(np.flipud(fpHist)))
...
def eval_image(gt_image, prob, cl_index, thresh):
    ...
    cl_gt = gt_image[:, :] == cl_index # cl_index对应的是要计算的类别
    FN, FP, posNum, negNum = evalExp(cl_gt, prob, thresh) # FN对应的是不同阈值下的假阴性
    return FN, FP, posNum, negNum
