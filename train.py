...
def various_distance(out_vec_t0, out_vec_t1,dist_flag):
    if dist_flag == 'l2': # 论文中采用的l2距离
        distance = F.pairwise_distance(out_vec_t0, out_vec_t1, p=2)
    if dist_flag == 'l1':
        distance = F.pairwise_distance(out_vec_t0, out_vec_t1, p=1)
    if dist_flag == 'cos': # 余弦相似度
...
def single_layer_similar_heatmap_visual(idx,output_t0,output_t1,save_change_map_dir,epoch,batch_idx,filename,layer_flag,dist_flag):
    n, c, h, w = output_t0.data.shape # 1,512,32,32 -> 1,32,256,256
    ...
    save_weight_fig_dir = os.path.join(save_change_map_dir_layer, str(batch_idx) + '_' + filename[0].split('/')[1])
    if idx % 20 == 0:
...
def validate(net, val_dataloader, epoch, batch_idx, save_change_map_dir, save_pr_dir, best_metric, best_epoch, best_batch_idx):
    ...
    with torch.no_grad():   
        ...
        # 减小阈值个数可以加速
        num_thresh = 96
        thresh = np.linspace(0.0, 2.2, num_thresh) # 这个是diff的阈值么？
        ...
        # mc.pxEval_maximizeFMeasure计算整个dataseth中最大的F_score值
...
def main():
    ...
    logging.basicConfig(filename=logname + '.txt', level=logging.DEBUG) # 创建一个文件保存训练的信息
    # configs
    best_metric = 0
    best_epoch = 0
    best_batch_idx = 0
    # load datasets
    train_transform_det = trans.Compose([trans.Scale(cfg.TRANSFROM_SCALES)]) # 如果尺寸相近不大，使用最近邻插值进行resize
    ...
    for epoch in range(start, cfg.MAX_EPOCH):
        for batch_idx, batch in enumerate(train_loader):
            ...
            step = epoch * 10000 + batch_idx  # 30 * 10000 + 1000，为何要乘以1万？
            ...
            out_conv5_t0, out_conv5_t1 = out_conv5                # (1, 512, 32, 32)  --> (1, 32, 256, 256)
            out_fc_t0, out_fc_t1 = out_fc                         # (1, 512, 32, 32)  --> (1, 32, 256, 256)
            out_embedding_t0, out_embedding_t1 = out_embedding    # (1, 512, 32, 32)  --> (1, 32, 256, 256)
            ...
            # 求3个loss
            contrastive_loss_conv5 = MaskLoss(out_conv5_t0, out_conv5_t1, label_rz_conv5) # 对空间注意力提取的特征图计算contrastive loss
            ...
            if batch_idx % 20 == 0: # 每20个batch打印一次loss
