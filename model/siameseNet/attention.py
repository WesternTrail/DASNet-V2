...
class PAM_Module(Module):
    ...
    def forward(self, x): # X：（b,512,32,32）
        ...
        # V
        proj_value = self.value_conv(x).view(m_batchsize, -1, width*height)  # (B, C=512, HxW)
...
class CAM_Module(Module):
    ...
    def forward(self,x):
        ...
        proj_key = x.view(m_batchsize, C, -1).permute(0, 2, 1)     # (B, HxW, 512) query和key仅仅是转置，不需要跟PAM里面一样通过conv
        energy = torch.bmm(proj_query, proj_key)                   # q*k (B, 512, 512)
