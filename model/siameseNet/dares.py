...
class DANetHead(nn.Module):
    ...
    def forward(self, x): # （B,2048,32,32）
        # 空间注意力分支
        feat1 = self.conv5a(x)
...
class DANet(BaseNet):
    ...
    def __init__(self, nclass, backbone, norm_layer=nn.BatchNorm2d, **kwargs):
        super(DANet, self).__init__(nclass, backbone, norm_layer=norm_layer, **kwargs) # 初始化resnet，512没用到
    ...
    def forward(self, x):
        ...
        x = self.head(c4) #返回
        x = list(x)
        return x[0], x[1], x[2] # 当前图像空间注意力提取的feature，通道注意力提取的feature
...
class SiameseNet(nn.Module):
    ...
    def forward(self, t0, t1):
        ...
        out_t0_conv5, out_t0_fc7, out_t0_embedding = self.CNN(t0) # 第一张图像的输出，包括空间注意力的特征，通道注意力的特征，预测的change map
        ...
        out_t0_conv5, out_t1_conv5 = self.decoder(out_t0_conv5), self.decoder(out_t1_conv5) # 空间注意力的特征表示
        out_t0_fc7, out_t1_fc7 = self.decoder(out_t0_fc7), self.decoder(out_t1_fc7) # (b,32,256,256) # 时间注意力的特征表示
        out_t0_embedding, out_t1_embedding = self.decoder(out_t0_embedding), self.decoder(out_t1_embedding) # 最后的结果(b,32,256,256)

        # 归一化t0 t1的sa # 为何要进行归一化？
